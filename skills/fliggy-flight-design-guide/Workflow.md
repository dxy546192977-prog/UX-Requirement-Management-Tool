...
本文件是**飞猪机票执行层主锚**：设计生成、审查、Figma 交付的真相源。上游由 **需求看板**（`online-index.html`，H5）负责产出 PRD 拆解 Markdown，中游为 `fliggy-flight-prd-to-h5` 编排与路由，下游为 Figma 精调。上中下游的细节不在此重复，仅以衔接小节与字段映射的方式保持一致。
...
**规则基线（强制）**：进入生成前必须先遵循 `skills/fliggy-flight-design-guide/playbooks/flight-funnel/0 Fliggy Design Skill/SKILL.md` 的读取与输出规范：
...
**中游路由衔接**：进入生成前必须先过一次 `skills/fliggy-flight-prd-to-h5/routing.md` 的路由算法，路由产出的以下字段必须带入生成上下文：
...
**审查同步规则**：审查结论必须与 `figma_handoff.review_summary` 保持一致；`key_issues > 0` 时 `figma_handoff.status` 禁止置为 `ready`；对标字段缺失同样禁止 `ready`（详见 `skills/fliggy-flight-prd-to-h5/figma-handoff-standard.md`）。

#### Phase 4：推送到 Figma

gstack browse 全页截图存档 → 通过 HTML to Figma 插件导入 Figma → 展示交付清单（含对标说明）→ 你在 Figma 中精调。

**交付契约引用**：交付包结构以 `skills/fliggy-flight-prd-to-h5/figma-handoff-standard.md` 为准，必须包含 `benchmark_source` / `benchmark_dimensions` / `benchmark_notes`，以及与审查结果对齐的 `review_summary` 和 `status`。
...
| `route_status` | `supported` / `partial` / `unsupported` | 中游（`fliggy-flight-prd-to-h5`） | Phase 2 进入前产出，`unsupported` 回退 Phase 0/1 |
...
| [`skills/fliggy-flight-prd-to-h5/SKILL.md`](../fliggy-flight-prd-to-h5/SKILL.md) | 编排层：总入口与输出协议 |
| [`skills/fliggy-flight-prd-to-h5/routing.md`](../fliggy-flight-prd-to-h5/routing.md) | 路由规则：页面支持度与降级策略 |
| [`skills/fliggy-flight-prd-to-h5/figma-handoff-standard.md`](../fliggy-flight-prd-to-h5/figma-handoff-standard.md) | 交付契约：Figma 交付包字段与状态机 |
| [`skills/fliggy-flight-prd-to-h5/google-awesome-design-baseline.md`](../fliggy-flight-prd-to-h5/google-awesome-design-baseline.md) | 对标基线：awesome-design 方法维度与追溯字段 |
| [`skills/fliggy-flight-design-guide/playbooks/flight-funnel/0 Fliggy Design Skill/SKILL.md`](playbooks/flight-funnel/0 Fliggy Design Skill/SKILL.md) | 生成规则基线：读取顺序、token、图片与输出约束 |
