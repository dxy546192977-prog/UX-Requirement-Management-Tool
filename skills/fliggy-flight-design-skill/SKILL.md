---
name: fliggy-flight-design-skill
version: 1.0.0
description: |
  飞猪机票 UX/设计执行 Skill：PRD 拆解接入 → 审查闸门 → 对标 → 单文件 H5 设计稿 → 审查 → Figma 交付。
...
1. **编排机票设计流水线**：从 PRD 拆解 Markdown 到可交付 HTML，再到审查与 Figma 交接语义（与上游看板、中游 `flight-prd-to-design` 衔接时，以 `references/workflow-master.md` 为流程真相源）。
...
外部编排（若已安装）：`flight-prd-to-design`、`prd-recognition`、项目内 `docs/*workflow*.md` 与本 Skill 并行阅读，以字段对齐表为准。
