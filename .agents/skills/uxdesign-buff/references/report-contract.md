...
```text
UXdesign-buff-reviews/<review-slug>/
```
...
- 如果需要渲染受限文案槽位，允许把 `.UXdesign-buff/<review-slug>/report-slots.json` 当作技术性 hidden scratch；它只能保存内容槽位，不能变成另一套 HTML 壳
...
- `<html lang="...">`
- `<meta name="UXdesign-buff-template" content="human-report-html-v1">`
- `<body data-UXdesign-buff-report="v1">`
...
- `report.html` 和 `review-state.json` 都在同一个 `UXdesign-buff-reviews/<review-slug>/` 下
...
以下报告段落必须包含实质性内容，不允许留"暂无。"或空白：

### 1. review-overview（评审总览）

- `.lede` 必须有 1-2 段概括性诊断文案，说清这份设计最核心的问题是什么

### 2. executive-summary（结构摘要）

- **当前判断**：必须有明确结论（能进开发/还需修改/有重大风险）
- **修正重点**：至少列出 1-3 项立即要动手的修正方向

### 3. resolution-tracks（结构摘要）中的每个子段

- **问题分布**：至少说明问题集中在哪些节点，或按维度/严重度的分布概况
- **修正批次建议**：按优先级分批的具体建议
- **关键风险**：至少 1 条核心风险，说明如果不修改会发生什么

### 4. background-and-evidence（评审依据）

- **评审依据**：必须说明审查了哪些页面/流程
- **评审边界**：必须说明可见证据的覆盖范围，以及判断的置信度水平

### 5. three-flow-consistency（三流一致性）中的"整体判断"

- 必须总结三条流之间是否互相支撑
- 如果不支撑，说明哪里断裂、如何互相放大风险

### 不可用"暂无。"的处理方式

如果确实无法获取某项信息，应写明原因，而不是简单写"暂无。"。

示例：
- ✗ "暂无。"
- ✓ "因截图未覆盖失败页面，无法评估错误恢复路径"
- ✓ "当前设计稿未包含站外回流截图，回流机制待确认"
...
- `scenario and fit`（场景匹配）—— 这一页是否真正理解并服务了它声称要服务的场景
- `cognitive and operational cost`（认知与操作成本）—— 用户完成任务需要付出的脑力和操作代价是否合理
- `mental model and habit`（用户心智与习惯）—— 设计是否尊重用户已经形成的认知习惯
- `value delivery`（价值承接）—— 用户在这一页能否清晰感知到价值，并被顺畅引导到下一步
- `baseline experience`（基础体验）—— 基础的交互质量和异常处理
...
```
.timeline-block
  ├─ 标题（h3 或 strong，可含方法论标注）
  ├─ "问题：" 标签
  ├─ "解法：" 标签
  └─ .methodology-source（可选，标注方法论来源）
```

方法论标注规则：

- `.methodology-source` 是可选元素，用于标注该检查项关联的方法论原则
- 渲染时方法论来源应以简洁的方式呈现（如括号注解或小字标签），不应喧宾夺主
- 示例：`<span class="methodology-source">Nielsen #4</span>` 或 `<span class="methodology-source">服务蓝图后台线</span>`
- 复合来源可以使用 `/` 分隔，例如 `Nielsen #4 / Sweller 认知负荷`
...
当内容包含并列的建议、步骤或清单时，每条独立成行，使用以下格式之一：
...
**禁止**：
> “建议第一批优先修复入口分流逻辑、失败页引导缺失、协议勾选视觉混淆”
...
特别注意 `resolution_tracks`（结构摘要）中的「修正批次建议」部分，它应该把每条建议作为独立的列表项，而不是用顿号连在一起。
...
```text
.UXdesign-buff/<review-slug>/report-slots.json
```
...
模板和合同中定义的 class 名都是合法的，包括：

- 时间轴相关：`timeline`、`timeline-node`、`timeline-stage`、`timeline-dot`、`timeline-stage-title`、`timeline-stage-summary`、`timeline-card`、`timeline-summary`、`timeline-block`
- 方法论标注：`methodology-source`
- 布局相关：`hero-panel`、`three-col`、`panel`、`actions-grid`
- 证据相关：`evidence-chips`
- 其他模板定义的合法 class
...
这份 HTML 报告是 `UXdesign-buff` 内嵌的一套小型设计系统。
...
hidden scratch 放在 `.UXdesign-buff/<review-slug>/` 下。
