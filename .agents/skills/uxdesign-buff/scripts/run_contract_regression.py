...
VALID_RENDER_STATE = (
    VALID_RENDER / "UXdesign-buff-reviews" / "self-check-render-sample" / "review-state.json"
)
VALID_RENDER_SLOTS = (
    VALID_RENDER / ".UXdesign-buff" / "self-check-render-sample" / "report-slots.json"
)

# Edge case fixtures
EDGE_EMPTY_ISSUES = FIXTURE_ROOT / "edge-empty-issues"
EDGE_EMPTY_ISSUES_STATE = (
    EDGE_EMPTY_ISSUES / "UXdesign-buff-reviews" / "empty-issues-sample" / "review-state.json"
)
EDGE_EMPTY_ISSUES_SLOTS = (
    EDGE_EMPTY_ISSUES / ".UXdesign-buff" / "empty-issues-sample" / "report-slots.json"
)

EDGE_MINIMAL_TIMELINE = FIXTURE_ROOT / "edge-minimal-timeline"
EDGE_MINIMAL_TIMELINE_STATE = (
    EDGE_MINIMAL_TIMELINE / "UXdesign-buff-reviews" / "minimal-timeline-sample" / "review-state.json"
)
EDGE_MINIMAL_TIMELINE_SLOTS = (
    EDGE_MINIMAL_TIMELINE / ".UXdesign-buff" / "minimal-timeline-sample" / "report-slots.json"
)

EDGE_MAX_TIMELINE = FIXTURE_ROOT / "edge-max-timeline"
EDGE_MAX_TIMELINE_STATE = (
    EDGE_MAX_TIMELINE / "UXdesign-buff-reviews" / "max-timeline-sample" / "review-state.json"
)
EDGE_MAX_TIMELINE_SLOTS = (
    EDGE_MAX_TIMELINE / ".UXdesign-buff" / "max-timeline-sample" / "report-slots.json"
)

EDGE_LONG_TITLES = FIXTURE_ROOT / "edge-long-titles"
EDGE_LONG_TITLES_STATE = (
    EDGE_LONG_TITLES / "UXdesign-buff-reviews" / "long-titles-sample" / "review-state.json"
)
EDGE_LONG_TITLES_SLOTS = (
    EDGE_LONG_TITLES / ".UXdesign-buff" / "long-titles-sample" / "report-slots.json"
)

VALID_ENGLISH_REPORT = FIXTURE_ROOT / "valid-english-report"
VALID_ENGLISH_REPORT_STATE = (
    VALID_ENGLISH_REPORT / "UXdesign-buff-reviews" / "english-report-sample" / "review-state.json"
)
VALID_ENGLISH_REPORT_SLOTS = (
    VALID_ENGLISH_REPORT / ".UXdesign-buff" / "english-report-sample" / "report-slots.json"
)
...
def render_fixture(state_path: Path, slots_path: Path, label: str) -> None:
    """Render a fixture and verify it succeeds."""
    ...
    require(result, True, f"render {label}")


def render_valid_fixture() -> None:
    render_fixture(VALID_RENDER_STATE, VALID_RENDER_SLOTS, "valid self-check fixture")
...
def mutate_h1_verdict(root: Path) -> None:
    report_path = root / "UXdesign-buff-reviews" / "self-check-render-sample" / "report.html"
...
def mutate_split_tail_sections(root: Path) -> None:
    ...
    old = """<section class="section">
      ...
      <p class="footer-note">产物路径：UXdesign-buff-reviews/self-check-render-sample/report.html · run_id: run_fixture_render_sample_001 · 评审节点: 36:587</p>
    </section>"""
...
def build_standalone_case(tmp_root: Path) -> Path:
    ...
    report_src = VALID_RENDER / "UXdesign-buff-reviews" / "self-check-render-sample" / "report.html"
...
def render_edge_fixtures() -> None:
    """Render all edge case fixtures."""
    render_fixture(EDGE_EMPTY_ISSUES_STATE, EDGE_EMPTY_ISSUES_SLOTS, "edge-empty-issues fixture")
    render_fixture(EDGE_MINIMAL_TIMELINE_STATE, EDGE_MINIMAL_TIMELINE_SLOTS, "edge-minimal-timeline fixture")
    render_fixture(EDGE_MAX_TIMELINE_STATE, EDGE_MAX_TIMELINE_SLOTS, "edge-max-timeline fixture")
    render_fixture(EDGE_LONG_TITLES_STATE, EDGE_LONG_TITLES_SLOTS, "edge-long-titles fixture")
    render_fixture(VALID_ENGLISH_REPORT_STATE, VALID_ENGLISH_REPORT_SLOTS, "valid-english-report fixture")


def main() -> int:
    # Render all fixtures first
    render_valid_fixture()
    render_edge_fixtures()

    # Validate original fixtures
    validate_fixture(VALID_GOLD, True, "gold fixture passes")
    validate_fixture(VALID_RENDER, True, "renderer fixture passes")

    # Validate edge case fixtures (all should pass)
    validate_fixture(EDGE_EMPTY_ISSUES, True, "edge-empty-issues passes")
    validate_fixture(EDGE_MINIMAL_TIMELINE, True, "edge-minimal-timeline passes (4 nodes)")
    validate_fixture(EDGE_MAX_TIMELINE, True, "edge-max-timeline passes (8 nodes)")
    validate_fixture(EDGE_LONG_TITLES, True, "edge-long-titles passes")
    validate_fixture(VALID_ENGLISH_REPORT, True, "valid-english-report passes")

    with tempfile.TemporaryDirectory(prefix="UXdesign-buff-contract-") as tmpdir:
