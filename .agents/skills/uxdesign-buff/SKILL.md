---
name: UXdesign-buff
description: UXdesign-buff 不是一个"审美打分器"，而是一个面向业务与体验的设计评审 Skill。它会基于 Figma、截图、页面结构和补充材料，快速重建设计背后的上下文：这是哪个场景、服务谁、要完成什么业务目标、处在产品链路的哪个位置。它看的不是一张静态图，而是一条真实会发生的用户路径。
...
- `UXdesign-buff/` 目录只放说明和模板，不承接运行时写入
- 正式输出写到 `UXdesign-buff-reviews/<review-slug>/`
- 技术性临时状态可以写到 `.UXdesign-buff/<review-slug>/`
...
> 在 Figma 中打开你想评审的画板，右键点击它，选择 "Copy link to selection"，然后把链接粘贴给我。

用户给了文件级 URL（没有 `node-id`）时的回复：

> 这个链接指向的是整个文件，我需要具体到某个画板。你可以在 Figma 左侧面板选中画板后右键复制链接。
>
> 或者，我可以帮你列出这个文件里的顶层画板，你告诉我要看哪个？

URL 格式不对时的回复：

> 这个链接格式不太对。Figma 链接通常长这样：
> `https://www.figma.com/design/xxx/xxx?node-id=xxx`
>
> 你可以试试在 Figma 里选中画板后右键 -> Copy link to selection。

### 输入容错

- 如果用户给了文件级 URL（没有 `node-id`），不直接报错。用 MCP 读取文件结构，列出顶层 frame 让用户选择，或者让用户确认要评审的范围。
- 如果用户一次给了多个 URL，支持批量 intake，按顺序处理或询问优先级。
- 如果 URL 解析失败，给出清晰的错误原因（比如「链接格式不对」或「这个文件我访问不了」）和修复指引。
...
- 用户明确要求机器友好输出
- 这轮输出的主要读者明确是另一个 agent、自动化步骤或下游流水线
- 用户明确要求把这次评审做成方便继续机器处理的 handoff

这些情况本身不构成切换理由：

- 当前运行时恰好是 agent / IDE / CLI
- 被审对象是 agent 生成的设计稿
- 你怀疑后续也许会被 agent 继续消费

只要主要读者仍然是设计师、设计 TL、PM 或业务同学，就继续保持 `self-check`。
...
- 渲染前先读取 `templates/report-shell.html`，把它当成固定 shell，而不是灵感来源；不要临时发明另一套章节骨架
- 先产出一个受限的 `report-slots.json` 内容槽位对象；它只能包含文案槽位，不得包含新的 section、class、CSS 或 HTML shell
- 正式 `report.html` 必须通过 `scripts/render_report.py --state UXdesign-buff-reviews/<review-slug>/review-state.json --slots .UXdesign-buff/<review-slug>/report-slots.json` 生成，不要直接手写最终 HTML
- `report_language` 一旦确定，标题、章节名、meta 标签、按钮样式文字和辅助说明都跟随同一语言；不要混入 `Executive Summary`、`Issue List`、`Resolution Tracks`、`Project`、`Review Slug` 这类英文脚手架，除非用户明确要求双语
- “说人话”的润色只允许发生在槽位文案里，不允许重排模板结构、替换 class、拆并 section，或把三流时间轴改写成卡片列表
- 落盘后立刻运行 `scripts/validate_review_contract.py <project-root>`；只要校验没过，就继续修到通过，再结束这轮输出

### 7. 持久化状态

正式写入：

- 更新 `UXdesign-buff-reviews/<review-slug>/report.html`
- 更新 `UXdesign-buff-reviews/<review-slug>/review-state.json`

hidden scratch 可以保存：

- JSON 状态
- `report-slots.json` 这类渲染槽位输入
- 运行元数据
- review unit 拆分结果
- read coverage
- evidence manifest
